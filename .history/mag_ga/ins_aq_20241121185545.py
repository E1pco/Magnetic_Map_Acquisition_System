...
def visualize_data():
    ...
    # 绘制角速度
    plt.subplot(3, 2, 2)
    plt.plot(chaptime_list, anglex_list, label='angle X', color='r')
    plt.plot(chaptime_list, angley_list, label='angle Y', color='g')
    plt.plot(chaptime_list, anglez_list, label='angle Z', color='b')
    plt.xlabel('Time (s)')
    plt.ylabel('angle (deg/s)')
    plt.title('Angle Rate Over Time')
    plt.legend()
    plt.grid()
