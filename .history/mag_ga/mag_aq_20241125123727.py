...
class SerialReader(threading.Thread):
    def __init__(self, ser, buffer, stop_event, plot_event):
        super().__init__()
        self.ser = ser
        self.buffer = buffer
        self.stop_event = stop_event
        self.plot_event = plot_event  # 用于传递解析后的数据
    ...
    def run(self):
        while not self.stop_event.is_set():
            if self.ser.in_waiting > 0:
                ...
                if x_list and y_list and z_list and t_list:  # 如果解析的列表非空
                    self.plot_event.put((x_list, y_list, z_list, t_list))  # 将解析后的数据放入队列
...
class DataProcessor(threading.Thread):
    def __init__(self, plot_event):
        super().__init__()
        self.plot_event = plot_event

    def run(self):
        while True:
            if not self.plot_event.empty():
                x_list, y_list, z_list, t_list = self.plot_event.get()
...
def visualize_data(x_values, y_values, z_values, t_values):
    ...
    plt.tight_layout()
    plt.show(block=False)  # 非阻塞显示以便实时更新
...
def send_and_read_from_serial(port, baudrate, send_data1, send_data2):
    ...
    reader = SerialReader(ser, buffer, stop_event, plot_event)
    processor = DataProcessor(plot_event)
