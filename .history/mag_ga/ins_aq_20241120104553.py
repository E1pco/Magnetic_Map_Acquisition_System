...
def visualize_data():
    ...
    # 绘制角速度
    plt.subplot(3, 2, 2)
    plt.plot(chaptime_list, anglex_list, label='Gyro X', color='r')
    plt.plot(chaptime_list, angley_list, label='Gyro Y', color='g')
    plt.plot(chaptime_list, anglez_list, label='Gyro Z', color='b')
    plt.xlabel('Time (s)')
    plt.ylabel('Gyro Rate (deg/s)')
    plt.title('Gyro Rate Over Time')
    plt.legend()
    plt.grid()
    ...
    # 绘制航向角和地速
    plt.subplot(3, 2, 5)
    plt.plot(chaptime_list, yaw_list, label='Yaw Angle', color='brown')
    plt.plot(chaptime_list, speed_list, label='Speed', color='pink')
    plt.xlabel('Time (s)')
    plt.ylabel('Yaw / Speed')
    plt.title('Yaw Angle and Speed Over Time')
    plt.legend()
    plt.grid()
...
if __name__ == '__main__':
    ...
    #读取配置信息 Read configuration information
