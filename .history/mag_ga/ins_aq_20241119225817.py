...
def onUpdate_uesr(deviceModel):
    ...
    if len(chaptime_list) > 1:  # 例如加一条数据后就绘制
        visualize_data()
...
def visualize_data():
    ...
    # 绘制加速度
    plt.subplot(3, 2, 1)
    plt.plot(chaptime_list, accx_list, label='加速度X轴', color='r')
    plt.plot(chaptime_list, accy_list, label='加速度Y轴', color='g')
    plt.plot(chaptime_list, accz_list, label='加速度Z轴', color='b')
    plt.xlabel('时间 (s)')
    plt.ylabel('加速度 (g)')
    plt.title('加速度随时间变化')
    plt.legend()
    ...
    # 绘制角速度
    plt.subplot(3, 2, 2)
    plt.plot(chaptime_list, anglex_list, label='角速度X轴', color='r')
    plt.plot(chaptime_list, angley_list, label='角速度Y轴', color='g')
    plt.plot(chaptime_list, anglez_list, label='角速度Z轴', color='b')
    plt.xlabel('时间 (s)')
    ...
    # 绘制温度变化
    plt.subplot(3, 2, 3)
    plt.plot(chaptime_list, temp_list, label='温度', color='purple')
    plt.xlabel('时间 (s)')
    plt.ylabel('温度 (°C)')
    plt.title('温度随时间变化')
    plt.legend()
    ...
    # 绘制其他数据，如经度和纬度
    plt.subplot(3, 2, 4)
    plt.plot(chaptime_list, lon_list, label='经度', color='orange')
    plt.plot(chaptime_lis024-11-19 22:56:06.079t, lat_list, label='纬度', color='cyan')
    plt.xlabel('时间 (s)')
    plt.ylabel('位置')
    plt.title('经纬度随时间变化')
    plt.legend()
    ...
    # 绘制航向角和地速
    plt.subplot(3, 2, 5)
    plt.plot(chaptime_list, yaw_list, label='航向角', color='brown')
    plt.plot(chaptime_list, speed_list, label='地速', color='pink')
    plt.xlabel('时间 (s)')
    plt.ylabel('航向角 / 地速')
    plt.title('航向角及地速随时间变化')
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.show()
