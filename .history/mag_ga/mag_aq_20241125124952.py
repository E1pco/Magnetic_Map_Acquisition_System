...
class SerialReader(threading.Thread):
    def __init__(self, ser, data_list, stop_event):
        super().__init__()
        self.ser = ser
        self.data_list = data_list  # 用于存储解析后的数据
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            if self.ser.in_waiting > 0:
                ...
                if len(t_list) > 0:  # 只在有数据时进行存储
                    # 将解析后的数据放入共享列表
                    for i in range(len(t_list)):
                        self.data_list.append((t_list[i], x_list[i], y_list[i], z_list[i]))
                # 清理已解析的数据
            ...
            if part.startswith("RD"):
                if i + 1 < len(parts):
                    ...
                    if len(xyz_parts) == 4:
                        ...
                        y = xyz_parts[2].strip()
                        z = xyz_parts[3].strip()
                        t_values.append(float(t))  # 转换为浮点数以便存储
...
def visualize_data(data_list):
    ...
    t_values, x_values, y_values, z_values = zip(*data_list)
...
def send_and_read_from_serial(port, baudrate, send_data1, send_data2):
    ...
    data_list = []  # 定义一个列表用于存储解析后的数据
    reader = SerialReader(ser, data_list, stop_event)
    reader.start()
    ...
    try:
        ...
        while not stop_event.is_set():
            if len(data_list) > 0:  # 确保列表中有数据
                visualize_data(data_list)  # 可视化数据
                data_list.clear()  # 清空已可视化的数据
